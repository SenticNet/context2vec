...
class Model:
    def __init__(self, cfg, vocab_counts):
        ...
        if not cfg.use_pretrained:
            self.word_embeddings = tf.get_variable(name="word_embeddings", shape=[cfg.vocab_size, cfg.word_dim],
                                                   dtype=tf.float32, trainable=True)
        else:
            self.word_embeddings = tf.Variable(np.load(cfg.pretrained_emb)["embeddings"], name="word_embeddings",
                                               dtype=tf.float32, trainable=cfg.tune_emb)
        self.word_embeddings = tf.concat([tf.zeros([1, cfg.word_dim]), self.word_embeddings[1:, :]], axis=0)

        self.verb_embeddings = tf.get_variable(name="verb_embeddings", shape=[cfg.verb_size, cfg.word_dim],
                                               dtype=tf.float32, trainable=True)
        ...
        with tf.variable_scope("left_context_representation"):
            ...
            h_lc, _ = bidirectional_dynamic_rnn(cell_fw, cell_bw, left_context_emb, sequence_length=self.left_seq_len,
                                                dtype=tf.float32, time_major=False, scope="bi_lstm")
            ...
            l_weight = tf.get_variable(name="l_weight", shape=[2 * cfg.num_units, 2 * cfg.num_units], dtype=tf.float32)
        ...
        with tf.variable_scope("right_context_representation"):
            cell_fw = LSTMCell(num_units=cfg.num_units)
            cell_bw = LSTMCell(num_units=cfg.num_units)
            h_rc, _ = bidirectional_dynamic_rnn(cell_fw, cell_bw, right_context_emb, sequence_length=self.right_seq_len,
                                                dtype=tf.float32, time_major=False, scope="bi_lstm")
            h_rc = tf.concat(h_rc, axis=-1)
            # self-attention
            h_rc = self_attention(h_rc, name="self_attn_right")
            r_weight = tf.get_variable(name="r_weight", shape=[2 * cfg.num_units, 2 * cfg.num_units], dtype=tf.float32)
        ...
        with tf.variable_scope("neural_tensor_network"):
            T = tf.get_variable(name="T", shape=[cfg.output_units, 2 * cfg.num_units, 2 * cfg.num_units],
                                dtype=tf.float32)
            W = tf.get_variable(name="W", shape=[4 * cfg.num_units, cfg.output_units], dtype=tf.float32)
            b = tf.get_variable(name="b", shape=[cfg.output_units], dtype=tf.float32)
            # compute tensors
        ...
        with tf.variable_scope("verb_representation"):
            ...
            # neg_verb_emb = tf.reshape(neg_verb_emb, shape=[-1, cfg.word_dim])
            negative_verbs = ffn_layer(neg_verb_emb, cfg.num_units, cfg.output_units, scope="ffn_layer")
            # negative_verbs = tf.reshape(negative_verbs, shape=[-1, cfg.neg_sample, cfg.output_units])
        ...
        with tf.variable_scope("compute_loss"):
            ...
            '''neg_logits = []
            for i in range(cfg.neg_sample):
                cur_logits = tf.reduce_sum(context * negative_verbs[:, i, :], axis=1)
                neg_logits.append(cur_logits)
            neg_logits = tf.reshape(tf.concat(neg_logits, axis=1), shape=[self.batch_size, cfg.neg_sample])'''


        with tf.variable_scope("loss"):
            '''nce_weights = tf.Variable(tf.truncated_normal([cfg.verb_size, cfg.output_units],
                                                          stddev=1.0 / math.sqrt(cfg.output_units)))
            nce_bias = tf.Variable(tf.zeros([cfg.verb_size]))
            # tf.expand_dims(self.verb, axis=1)
            self.losses = tf.reduce_mean(tf.nn.nce_loss(weights=nce_weights, biases=nce_bias,
                                                        labels=tf.expand_dims(self.verb, axis=1),
                                                        inputs=context, num_sampled=cfg.neg_sample,
                                                        num_classes=cfg.verb_size))'''
            self.loss = self.nce_loss(true_logits, neg_logits, self.batch_size)
    ...
    def nce_loss(self, true_logits, sampled_logits, batch_size):
        ...
        nce_loss_tensor = (tf.reduce_sum(true_xent) + tf.reduce_sum(sampled_xent)) / tf.cast(batch_size, dtype=tf.float32)
        return nce_loss_tensor

    def get_feed_dict(self, data, is_train=False, drop_rate=0.0, lr=None):
        feed_dict = {self.verb: data["vb"], self.left_context: data["lc"], self.left_seq_len: data["ll"],
                     self.right_context: data["rc"], self.right_seq_len: data["rl"],
